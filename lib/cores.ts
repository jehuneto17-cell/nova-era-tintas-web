...
import type { CorTinta } from "./types";
...
/** Paleta global de cores da loja (catálogo Suvinil), usada por produtos com `todasCores: true`. */
export async function getPaletaCores(): Promise<CorTinta[]> {
  const q = query(
    collection(db, "cores"),
    where("ativa", "==", true),
    orderBy("familia", "asc"),
...
/** Agrupa a paleta por família, na mesma ordem visual do admin. */
export function agruparPorFamilia(cores: CorTinta[]): { familia: string; cores: CorTinta[] }[] {
  const grupos = new Map<string, CorTinta[]>();
  cores.forEach((c) => {
    const lista = grupos.get(c.familia) ?? [];
    lista.push(c);
    grupos.set(c.familia, lista);
  });
